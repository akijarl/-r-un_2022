...
```{r dálkar}
#spendyr=read.csv("spendyr.csv",sep=";", dec=”,”) 
#ath hjálp fyrir read.csv
#fallið gerir ráð fyrir header, sep=",", dec=".", þessu þarf     # kannski að breyta,  einnig væri hægt að nota                    # "Import Dataset"" takkann
#head(spendyr)
##         tegund   likami heili                                ## 1      Refur        3.385  44.5
## 2      Uglu-api     0.480  15.5
## 3      Bifur        1.350   8.1
## 4      Kyr        465.000 423.0
## 5      Ulfur       36.330 119.5
## 6      Geit         27.660 115.0
#names(spendyr) # ég var með fleiri gögn
## [1] "tegund" "likami"    "heili"
#plot(spendyr$likami,spendyr$heili) 

# betra væri að umbreyta gögnunum t.d. með logarithma (log)
#plot(log(spendyr$likami),log(spendyr$heili)) 
# finnum bestu beinu línu, notum fallið lm
#heili.lm=lm(log(heili)~log(likami),data=spendyr)
#heili.lm # hér má finna skurðpunkt (a) og hallatölu línunnar (b)
## 
## Call:
## lm(formula = log(heili) ~ log(likami), data = spendyr)
## 
## Coefficients:
## (Intercept)    log(body)  
##      2.1348       0.7517
...
#notið identify(spendyr$likami,log(spendyr$heili),spendyr$tegund)
# klikkið á punktana og síðan á "Finish" takkann í horninu til   # hægri fyrir ofan myndina
# Hér mætti breyta textanum á ásunum með xlab og ylab
```
